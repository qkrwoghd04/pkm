---
id: agents
id_aliases:
  - public/agents
title: Agents
description: 운영 중인 에이전트의 목적, 구조, 행동 규칙, 설정, 검증, 자동화, 런북을 연결하는 지식 지도.
status: active
updated: 2026-07-30
aliases:
  - 에이전트
tags:
  - domain/agent-automation
  - tech/openclaw
  - concern/navigation
---

> [!summary]
> 에이전트별 현재 동작과 운영 지식을 찾는 출발점이다. 공유 인프라와 외부 연동은 각각 Systems와 Integrations에서 관리한다.

## Agent maps

- [[calendar/index|Calendar Agent]] — Google Calendar 일정 관리와 Telegram 연동
- [[market/index|Market Agent]] — 시장 조사, 평가, 아침 브리프 자동화

## Related knowledge

- [[../systems/index|Systems]]
- [[../integrations/index|Integrations]]
- [[../playbooks/index|Playbooks]]
- [[../incidents/index|Incidents]]
